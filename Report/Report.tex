...
The research question posed in this project, along with its clarification will be discussed in Chapter \ref{chap:researchquestion}. This is then followed by the elaboration on the concept in Chapter \ref{chap:concept}, where we will discuss the visual design choices and method, and our MoSCoW analysis method. The technical details of the software architecture will be discussed in Chapter \ref{chap:softwarearchitecture}, detailing the concept of the communication between different servers and our choice of dataset. The heart of our classification, the neural network, will then be elaborated upon in Chapter \ref{chap:CNN}, with the method of implementation of this network in Chapter \ref{chap:method}. Finally, the results and discussion will be described in detail in Chapter \ref{chap:resultsanddiscussion}.
...
\\
In order to classify the satellite image, the map is first divided into patches of predetermined dimensions. 
These patches are squares, the dimensions should be divisible by 2 and these are the main, large patches to be fed into the neural network. 
The network will label the patches and return the outcome, resulting in a grid-like classification as shown in Figure \ref{fig:impression}. 
\\
However, we wanted to give the user the choice of making a more detailed classification with a smaller grid size. 
To do so, an additional grid overlay is created with a small modification to the original algorithm: the large patches now overlap each other in both dimensions by a half. 
So if the map can be divided into N large patches in the width and M large patches in the height, this new setup will now have (2N - 1) patches in the width and (2M - 1) patches in the height, discarding patches that are halfway out of bounds. 
These patches are then fed to the neural network in the exact same fashion as the previous method. 
A second grid overlay is then effectively created, with the size of these smaller patches exactly a quarter of the larger patches. 
Figure \ref{fig:grid} provides an visual aid of this process, with the green patch indicating the large patch and the red patch indicating the resulting smaller patch. 
\\
The idea is to link these smaller patches to its four larger patches, these are its labeled parents. 
Using a majority vote, the labels of these smaller patches can be determined using its corresponding parent patches. 
For example, if three of its parent patches is labeled as city and only one parent is classified as forest, the label of the small patch will be city. 
This results in a new classification grid with four times the amount patches, but a quarter of the size of a larger patch. 
This in turn allows the user to make a finer classification and switch back and forth accordingly.
