...
For example, if there are two possible classes, A and B, the certainty values of a large patch can be denoted as $\{a_1, b_1\}$. The values of the four parents of a small patch are then summed, so you end up with $\{A, B\}$. If $A > B$, then the small patch will be labeled as $A$, otherwise it's labeled as $B$.
