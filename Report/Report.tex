...
In the second phase we tried to extend our classes with the class \textit{water}. We tried to investigate whether training only the last weights and biases were sufficient to train the network to classify three classes. During this phase we made some adjustment to the network. We realised that the last sub-sampling with a dimension reduction of a factor 5 would probably lead to less accuracy by the network since more information might be lost in this reduction. Although, we did no quantitative research on this hypothesis the underlaying reasoning is that in each layer of the network more generalizations are made by the network which should represent the information of your input patch. When you downsize the feature maps at the end of the network this more contained information will be lost in this sub-sampling compared with sub-sampling earlies in the network. Therefore we introduced a slightly different CNN. Now, the inputsize of the patches is 41x41. This is convolved with a 9x9 convolution kernel resulting in a 33x33 feature map. Now sub-sampling with a factor 3 is introduced reducing it to a 11x11 feature map. From this map we can branch off (H3) with a convolution operation with a 4x4 convolution kernel resulting in a 8x8 feature map. This last map is than sub-sampled with a reduction of 2 resulting in the output map of 4x4. In this way we presume that the network makes better generalization than the earlier network. 
\\\\
